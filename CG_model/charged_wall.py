...
# Store the coordinates of ions within 50 angstrom from the tunnel centerline derived from PyMOL
ion_coordinates = []

# Read and process the file
with open(f'/Users/Desktop/50_ion_{molecule}.txt', 'r') as file:
    coordinates = []
    for line in file:
        cleaned_line = re.sub(r"\\", "", line).strip()
        coords = tuple(map(float, cleaned_line.split()))
...
# Remove duplicate coordinates
ion_coordinates = list(map(list, set(coordinates)))
print(len(ion_coordinates),ion_coordinates[0])
