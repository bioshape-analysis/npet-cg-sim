...
def visualize_clipped_mesh(
    mesh_path: str, base_point: np.ndarray, axis_point: np.ndarray, angle: float
):
    """
    Visualize the clipping result with both halves in different colors.
    """
    # Get both halves of the mesh
    half1, half2 = clip_mesh_at_angle(
        mesh_path, base_point, axis_point, angle, return_both=True
    )
    # Create plotter
    p = pv.Plotter()
...
def clip_mesh_with_normal_correction(
    mesh_path: str,
    base_point: np.ndarray,
    axis_point: np.ndarray,
    angle: float,
...
if __name__ == "__main__":
    ...
    fixed_mesh = clip_mesh_with_normal_correction(
        meshpath, base_point, axis_point, 45, clippedpath
    )
